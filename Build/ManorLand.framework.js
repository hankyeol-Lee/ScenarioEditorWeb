...
// end include: preamble.js
  ...
  function _DownloadTextFile(filenamePtr, contentPtr) {
          var filename = UTF8ToString(filenamePtr);
          var content = UTF8ToString(contentPtr);
  
          // 텍스트 내용을 담은 Blob 객체 생성
          var blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
          var url = window.URL.createObjectURL(blob);
  
          // 가상의 a 태그 생성 및 다운로드 속성 부여
  ...
  function _UploadImageFiles(objectNamePtr, methodNamePtr) {
          var objectName = UTF8ToString(objectNamePtr);
          var methodName = UTF8ToString(methodNamePtr);
  
          var fileInput = document.createElement('input');
          fileInput.setAttribute('type', 'file');
          // 이미지 파일만 다중 선택 가능하도록 설정
              ...
              for (var i = 0; i < files.length; i++) {
                  (function(file) {
                      ...
                      reader.onload = function (e) {
                          ...
                          // 확장자를 제외한 순수 파일명 추출 (소문자로 통일)
                          var fileName = file.name.split('.')[0].toLowerCase(); 
                          
                          // "파일명|Base64데이터" 형태로 유니티에 전송
                          SendMessage(objectName, methodName, fileName + "|" + base64);
                      };
  ...
  function _UploadTextFile(objectNamePtr, methodNamePtr) {
          var objectName = UTF8ToString(objectNamePtr);
          var methodName = UTF8ToString(methodNamePtr);
  
          // 가상의 file input 태그 생성
          var fileInput = document.createElement('input');
          fileInput.setAttribute('type', 'file');
...
var wasmImports = {
  ...
  "UploadImageFiles": _UploadImageFiles,
  "UploadTextFile": _UploadTextFile,
